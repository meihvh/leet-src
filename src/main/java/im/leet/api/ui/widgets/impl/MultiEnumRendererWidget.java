...
public class MultiEnumRendererWidget
extends UIWidget {
    MultiEnumRenderer<?> parent;
    float scroll;
    float scrollAnim;

    public MultiEnumRendererWidget(MultiEnumRenderer<?> parent) {
        this.parent = parent;
    ...
    public void render(int mouseX, int mouseY) {
        ...
        Client.RENDERER.text(String.format("Selected %s / %s", ((MultiEnumSetting)this.parent.getSetting()).get().size(), ((MultiEnumSetting)this.parent.getSetting()).getEnumEntries().size()), this.x + 4.0f, this.y + 3.5f, TextureUse.SFMEDIUM, 7.0f, ClientColors.FORE_COLOR);
        float offY = 15.0f;
        float out = this.animation.getOutput();
        system.push(this.x, this.y + 15.0f, this.width, this.height - 15.0f);
        for (MultiEnumRenderer.MultiObject o : this.parent.getObjects()) {
    ...
    public boolean click(int mouseX, int mouseY, int button) {
        ...
        for (MultiEnumRenderer.MultiObject o : this.parent.getObjects()) {
            ...
            ((MultiEnumSetting)this.parent.getSetting()).toggle(((Enum)o.entry).ordinal());
            break;
        }
        return true;
    }
